...
//예외(에러) 처리 구조체  //(PowError가 Error 인터페이스를 구현한다면, Error처리를 담당하는 구조체라고 볼 수 있다. 덕타이핑 )
...
func main() {
  ...
  //예제2
  t, err := Power(0, 3) //정상처리
  if err != nil {
    // log.Fatal(err)
    log.Fatal(err.Error())
    fmt.Println(err.(PowError).value) //interface_ex2.go 참고
