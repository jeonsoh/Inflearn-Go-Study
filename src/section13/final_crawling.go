...
func scrapContents(url string, fn string) {
	...
	//파일 스크립 생성(열기) -> 파일명, 옵션, 권한
	file, err := os.OpenFile("./scrape/"+fn+".txt", os.O_CREATE|os.O_RDWR, os.FileMode(0777))
	errCheck(err)
	...
	//matchNode메소드를 사용해서 원하는 노드 순회(Iterator)하면서 출력
...
func main() {
	...
	//ParseMainNodes 메소드를 사용하여 크롤링(스크랩핑) 대상 URL 추출
	...
	for _, link := range urlList {
		...
		// fmt.Println(strings.Replace("oink oink oink oink", "k", "ky", 2)) //replace 사용법
